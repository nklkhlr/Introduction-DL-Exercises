...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dh, dw2, db2 = affine_backward(dout, score_cache)
        dx, dw1, db1 = affine_relu_backward(dh, h_cache)

        loss += 0.5 * self.reg * sum(np.sum(W**2) for W in (self.params['W1'], self.params['W2']))
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        if not isinstance(hidden_dims, int):
            ...
            raise AttributeError('You attempt to create a model with no hidden layer. However this class requires at least one hidden layer.')
        ############################################################################
    ...
    def loss(self, X, y=None):
        ...
        # affine - relu for first to second last layer
        H_i = X.copy()

        if self.use_batchnorm:
            for i in np.arange(1, self.num_layers - 1):
                H_i, self.cache[i] = affine_bn_relu_forward(H_i, self.params['W'+str(i)],
                                                    self.params['b'+str(i)], self.params['gamma'+str(i)],
                                                    self.params['beta'+str(i)], self.bn_params[i])
                H_i, self.dropout_param[i] = dropout_forward(H_i, self.dropout_param)
        else:
            for i in np.arange(1, self.num_layers - 1):
                H_i, self.cache[i] = affine_relu_forward(H_i, self.params['W'+str(i)],
                                                         self.params['b'+str(i)])

        # scores
        scores, score_cache = affine_forward(H_i, self.params['W'+str(self.num_layers-1)],
        ...
        ############################################################################
        grads = {}
        # compute softmax loss and derivative
        loss, dout = softmax_loss(scores, y)
        loss += 0.5 * self.reg * sum(np.sum(self.params['W'+str(i)]**2) \
                                        for i in np.arange(1, self.num_layers))


        # compute derivatives and update gradients of first layer
        dout, dw, db = affine_backward(dout, score_cache)
        grads['W'+str(self.num_layers-1)] = dw + self.reg*self.params['W'+str(self.num_layers-1)]
        grads['b'+str(self.num_layers-1)] = db

        # loop over over all other layers in reverse order
        i = self.num_layers - 2
        if self.use_batchnorm:
            while i >= 1:
                dout = dropout_backward(dout, self.dropout_param[i])
